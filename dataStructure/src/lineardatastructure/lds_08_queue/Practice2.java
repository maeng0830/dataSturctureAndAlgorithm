package lineardatastructure.lds_08_queue;// Practice2
...
class MyQueue2 {
    ...
    MyQueue2(int size) {
        arr = new int[size + 1]; // 첫번째 공간은 빈 상태일 것이기 때문에 n + 1 크기의 배열이 필요함.
    }

    public boolean isEmpty() {
        return this.rear == this.front; // 원형큐 관리를 위해서는 첫번쨰 공간(front)는 비워둘 것이다. 데이터는 두번째 공간부터 채워질 것이다(rear).
    }
//
